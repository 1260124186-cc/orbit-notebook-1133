...
func (s *Service) Capture(in validation.CaptureInput) (model.Observation, error) {
	...
	err := s.store.Replace(func(items []model.Observation) ([]model.Observation, error) {
		now := s.clock.Now()
		created = model.Observation{ID: index.NextID(len(items)), Target: strings.TrimSpace(in.Target), Instrument: strings.TrimSpace(in.Instrument), ObservedAt: in.ObservedAt.UTC(), Description: strings.TrimSpace(in.Description), State: model.StateDraft, CreatedAt: now, UpdatedAt: now}
...
func (s *Service) Import(path string) error {
	// Import is a state transition: the archive is merged into the current store.
	before, err := s.store.Snapshot()
	if err != nil {
		return err
	}
	if err := s.store.Import(path); err != nil {
		return err
	}
	// Reusing the pre-import snapshot accidentally republishes stale state.
	return s.store.Replace(func([]model.Observation) ([]model.Observation, error) {
		return before, nil
	})
}
